...
namespace UltimateCC
{
    public class PlayerJumpState : MainState, IMove2D
    {
        ...
        private PlayerData.JumpVariables.JumpInf JumpType => jumpInfo.currentJumpType;
        ...
        public override void SwitchStateLogic()
        {
            ...
            else if (playerData.Physics.IsNextToWall)
            {
                ...
                else if (localTime > playerData.Jump.IgnoreWallSlideTime && inputManager.Input_Walk != 0 &&
                         Mathf.Sign(inputManager.Input_Walk) == Mathf.Sign(playerData.Physics.WallDirection))
                {
        ...
        {
            float XVelocity;
            if (inputManager.Input_Walk != 0 && (localXVelovity == 0 ||
                                                 Mathf.Sign(inputManager.Input_Walk) == Mathf.Sign(localXVelovity))
                                             && (phase != Phase.TurnBack || xCurveTime > playerData.Walk.TurnBackTime))
            {
                ...
                XVelocity *= inputManager.Input_Walk * playerData.Jump.CurrentJump.jumpXImpulse;
                     ...
                     ((localXVelovity != 0 && Mathf.Sign(inputManager.Input_Walk) != Mathf.Sign(localXVelovity)) ||
            ...
            else if (localXVelovity != 0)
            {
                ...
                XVelocity *= playerData.Jump.CurrentJump.jumpXImpulse * playerData.Physics.FacingDirection * (int)JumpType.JumpDirection;
            }
            else
            {
